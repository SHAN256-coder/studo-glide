...
const StatusPage = () => {
  ...
  const renderApp = (app: typeof apps[0], i: number) => {
    ...
    return (
      <motion.div key={app.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-card p-3 space-y-1.5">
        <div className="flex items-center justify-between gap-2">
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${config.className}`}>{config.label}</span>
  ...
  return (
    <div className="space-y-4 pb-20 sm:pb-6">
      ...
      {apps.length === 0 ? (
        <p className="text-center text-muted-foreground text-sm py-12">No applications found.</p>
      ) : (
        <div className="grid grid-cols-2 gap-3">
          <div>
            <p className="text-xs font-semibold text-primary mb-2">🏠 Hosteller</p>
            <div className="space-y-2">{hostellerApps.length > 0 ? hostellerApps.map(renderApp) : <p className="text-[10px] text-muted-foreground text-center py-4">No hosteller applications</p>}</div>
          </div>
          <div>
            <p className="text-xs font-semibold text-primary mb-2">🎓 Day Scholar</p>
            <div className="space-y-2">{dayScholarApps.length > 0 ? dayScholarApps.map(renderApp) : <p className="text-[10px] text-muted-foreground text-center py-4">No day scholar applications</p>}</div>
          </div>
        </div>
      )}
    </div>
